#!/usr/bin/env bash
# Relatório pós-instalação - Layout 2 colunas

get_version() {
  local cmd="$1"
  case "$cmd" in
    git) git --version 2>&1 | awk '{print $3}' ;;
    zsh) zsh --version 2>&1 | awk '{print $2}' ;;
    fish) fish --version 2>&1 | awk '{print $3}' ;;
    tmux) tmux -V 2>&1 | awk '{print $2}' ;;
    nvim) nvim --version 2>&1 | head -n 1 | awk '{print $2}' ;;
    starship) starship --version 2>/dev/null | head -n 1 | awk '{print $2}' ;;
    mise) mise --version 2>/dev/null | head -n 1 | awk '{print $1}' ;;
    code) code --version 2>&1 | head -n 1 ;;
    docker) docker --version 2>&1 | awk '{print $3}' | tr -d ',' ;;
    node) node --version 2>/dev/null ;;
    python) python3 --version 2>/dev/null | awk '{print $2}' ;;
    php) php --version 2>/dev/null | head -n 1 | awk '{print $2}' ;;
    rust) rustc --version 2>/dev/null | awk '{print $2}' ;;
    go) go version 2>/dev/null | awk '{print $3}' | sed 's/go//' ;;
    bun) bun --version 2>/dev/null ;;
    deno) deno --version 2>/dev/null | head -n 1 | awk '{print $2}' ;;
  esac
}

print_post_install_report() {
  local username="${USER:-$(whoami)}"
  local hostname="${HOSTNAME:-$(hostname 2>/dev/null || echo 'localhost')}"

  # Cores
  local RED='\033[0;31m'
  local GREEN='\033[0;32m'
  local YELLOW='\033[1;33m'
  local BLUE='\033[0;34m'
  local PURPLE='\033[0;35m'
  local CYAN='\033[0;36m'
  local WHITE='\033[1;37m'
  local NC='\033[0m'

  [[ -t 1 ]] && clear

  # ═══════════════════════════════════════════════════════════════════════════
  # HEADER
  # ═══════════════════════════════════════════════════════════════════════════
  echo ""
  echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${GREEN}║${NC}                    ${YELLOW}✨ INSTALAÇÃO CONCLUÍDA COM SUCESSO! ✨${NC}                    ${GREEN}║${NC}"
  echo -e "${GREEN}║${NC}                      ${WHITE}Seu ambiente de desenvolvimento está pronto${NC}                   ${GREEN}║${NC}"
  echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════════════════════╝${NC}"
  echo ""

  # ═══════════════════════════════════════════════════════════════════════════
  # INFORMAÇÕES DO SISTEMA (compacto)
  # ═══════════════════════════════════════════════════════════════════════════
  echo -e "${BLUE}📊 INFORMAÇÕES DA INSTALAÇÃO${NC}"
  echo -e "${CYAN}┌─────────────────────────────────────────────────────────────────────────────────┐${NC}"
  echo -e "${CYAN}│${NC} ${WHITE}👤 Usuário:${NC} $username          ${WHITE}🖥️  Host:${NC} $hostname          ${WHITE}🐧 OS:${NC} ${TARGET_OS:-linux}"
  echo -e "${CYAN}│${NC} ${WHITE}📂 Origem:${NC}  ${SCRIPT_DIR:-$(dirname "$(realpath "$0")")}"
  echo -e "${CYAN}│${NC} ${WHITE}📁 Destino:${NC} $HOME"
  [[ -d "$BACKUP_DIR" ]] && echo -e "${CYAN}│${NC} ${WHITE}📦 Backup:${NC}  $BACKUP_DIR"
  echo -e "${CYAN}└─────────────────────────────────────────────────────────────────────────────────┘${NC}"
  echo ""

  # ═══════════════════════════════════════════════════════════════════════════
  # FERRAMENTAS INSTALADAS (2 colunas)
  # ═══════════════════════════════════════════════════════════════════════════
  echo -e "${PURPLE}🛠️  FERRAMENTAS INSTALADAS${NC}"
  echo -e "${CYAN}┌───────────────────────────────────────┬─────────────────────────────────────────┐${NC}"

  # Coletar ferramentas
  local col1=() col2=()
  has_cmd git && col1+=("📚 Git: $(get_version git)")
  has_cmd zsh && col1+=("🐚 Zsh: $(get_version zsh)")
  has_cmd fish && col1+=("🐟 Fish: $(get_version fish)")
  has_cmd tmux && col1+=("📺 Tmux: $(get_version tmux)")
  has_cmd nvim && col1+=("📝 Neovim: $(get_version nvim)")

  has_cmd starship && col2+=("🚀 Starship: $(get_version starship)")
  has_cmd code && col2+=("💻 VS Code: $(get_version code)")
  has_cmd docker && col2+=("🐳 Docker: $(get_version docker)")
  has_cmd mise && col2+=("📦 Mise: $(get_version mise)")

  local max=${#col1[@]}
  [[ ${#col2[@]} -gt $max ]] && max=${#col2[@]}

  for (( i=0; i<max; i++ )); do
    local left="${col1[i]:-}"
    local right="${col2[i]:-}"
    printf "${CYAN}│${NC} ${WHITE}%-37s${NC} ${CYAN}│${NC} ${WHITE}%-39s${NC} ${CYAN}│${NC}\n" "$left" "$right"
  done

  echo -e "${CYAN}└───────────────────────────────────────┴─────────────────────────────────────────┘${NC}"
  echo ""

  # ═══════════════════════════════════════════════════════════════════════════
  # RUNTIMES (se houver, em 2 colunas)
  # ═══════════════════════════════════════════════════════════════════════════
  local rt_col1=() rt_col2=()

  if has_cmd node || has_cmd python3 || has_cmd php; then
    has_cmd node && rt_col1+=("🟢 Node.js: $(get_version node)")
    has_cmd python3 && rt_col1+=("🐍 Python: $(get_version python)")
    has_cmd php && rt_col2+=("🐘 PHP: $(get_version php)")
    has_cmd rustc && rt_col2+=("🦀 Rust: $(get_version rust)")
    has_cmd go && rt_col2+=("🔷 Go: $(get_version go)")
    has_cmd bun && rt_col2+=("🧅 Bun: $(get_version bun)")
    has_cmd deno && rt_col2+=("🦕 Deno: $(get_version deno)")

    echo -e "${YELLOW}🚀 RUNTIMES DE DESENVOLVIMENTO${NC}"
    echo -e "${CYAN}┌───────────────────────────────────────┬─────────────────────────────────────────┐${NC}"

    local rt_max=${#rt_col1[@]}
    [[ ${#rt_col2[@]} -gt $rt_max ]] && rt_max=${#rt_col2[@]}

    for (( i=0; i<rt_max; i++ )); do
      local left="${rt_col1[i]:-}"
      local right="${rt_col2[i]:-}"
      printf "${CYAN}│${NC} ${WHITE}%-37s${NC} ${CYAN}│${NC} ${WHITE}%-39s${NC} ${CYAN}│${NC}\n" "$left" "$right"
    done

    echo -e "${CYAN}└───────────────────────────────────────┴─────────────────────────────────────────┘${NC}"
    echo ""
  fi

  # ═══════════════════════════════════════════════════════════════════════════
  # PRÓXIMOS PASSOS (lista vertical limpa)
  # ═══════════════════════════════════════════════════════════════════════════
  echo -e "${GREEN}⚡ PRÓXIMOS PASSOS${NC}"
  echo -e "${CYAN}┌─────────────────────────────────────────────────────────────────────────────────┐${NC}"
  echo -e "${CYAN}│${NC}                                                                                 ${CYAN}│${NC}"
  echo -e "${CYAN}│${NC}  ${WHITE}1.${NC} Reinicie o terminal:              ${BLUE}exec \$SHELL${NC}                              ${CYAN}│${NC}"
  if [[ ${INSTALL_POWERLEVEL10K:-0} -eq 1 ]]; then
    echo -e "${CYAN}│${NC}  ${WHITE}2.${NC} Configure Powerlevel10k:          ${BLUE}p10k configure${NC}                           ${CYAN}│${NC}"
  fi
  if [[ ${#SELECTED_NERD_FONTS[@]} -gt 0 ]]; then
    echo -e "${CYAN}│${NC}  ${WHITE}3.${NC} Configure a fonte no terminal:    ${GREEN}${SELECTED_NERD_FONTS[0]} Nerd Font${NC}                    ${CYAN}│${NC}"
  fi
  if [[ ${GIT_CONFIGURE:-0} -eq 1 ]]; then
    echo -e "${CYAN}│${NC}  ${WHITE}4.${NC} Clone projetos nas pastas:        ${BLUE}~/personal/*${NC} ou ${BLUE}~/work/*${NC}                  ${CYAN}│${NC}"
  fi
  echo -e "${CYAN}│${NC}                                                                                 ${CYAN}│${NC}"
  echo -e "${CYAN}└─────────────────────────────────────────────────────────────────────────────────┘${NC}"
  echo ""

  # ═══════════════════════════════════════════════════════════════════════════
  # COMANDOS ÚTEIS (lista vertical clara)
  # ═══════════════════════════════════════════════════════════════════════════
  echo -e "${YELLOW}💡 COMANDOS ÚTEIS${NC}"
  echo -e "${CYAN}┌─────────────────────────────────────────────────────────────────────────────────┐${NC}"
  echo -e "${CYAN}│${NC}                                                                                 ${CYAN}│${NC}"
  echo -e "${CYAN}│${NC}  ${WHITE}bash install.sh export${NC}  →  Salvar mudanças nos dotfiles                       ${CYAN}│${NC}"
  echo -e "${CYAN}│${NC}  ${WHITE}bash install.sh sync${NC}    →  Sincronizar dotfiles (pull + install)              ${CYAN}│${NC}"
  if has_cmd fzf; then
    echo -e "${CYAN}│${NC}  ${WHITE}Ctrl+R${NC}                  →  Buscar histórico com fzf                           ${CYAN}│${NC}"
  fi
  if has_cmd lazygit; then
    echo -e "${CYAN}│${NC}  ${WHITE}lazygit${NC}                 →  Interface TUI para Git                             ${CYAN}│${NC}"
  fi
  if has_cmd zoxide; then
    echo -e "${CYAN}│${NC}  ${WHITE}z <pasta>${NC}               →  Navegação inteligente (zoxide)                     ${CYAN}│${NC}"
  fi
  echo -e "${CYAN}│${NC}                                                                                 ${CYAN}│${NC}"
  echo -e "${CYAN}└─────────────────────────────────────────────────────────────────────────────────┘${NC}"
  echo ""

  # ═══════════════════════════════════════════════════════════════════════════
  # DICAS DO MISE (se instalado)
  # ═══════════════════════════════════════════════════════════════════════════
  if has_cmd mise; then
    echo -e "${PURPLE}📦 MISE - GERENCIADOR DE RUNTIMES${NC}"
    echo -e "${CYAN}┌─────────────────────────────────────────────────────────────────────────────────┐${NC}"
    echo -e "${CYAN}│${NC}                                                                                 ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}  ${WHITE}mise ls${NC}                   →  Listar runtimes instalados                        ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}  ${WHITE}mise ls-remote node${NC}       →  Ver versões disponíveis do Node.js                ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}  ${WHITE}mise use -g node@22${NC}       →  Instalar e definir Node 22 como global            ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}  ${WHITE}mise use node@20${NC}          →  Usar Node 20 apenas no projeto atual              ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}  ${WHITE}mise install${NC}              →  Instalar versões definidas no .mise.toml          ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}  ${WHITE}mise prune${NC}                →  Remover versões não utilizadas                    ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}                                                                                 ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}  ${BLUE}Exemplos:${NC}                                                                       ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}    mise use -g python@3.12     →  Python 3.12 global                            ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}    mise use -g rust@stable     →  Rust stable global                            ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}    mise use -g go@1.22         →  Go 1.22 global                                ${CYAN}│${NC}"
    echo -e "${CYAN}│${NC}                                                                                 ${CYAN}│${NC}"
    echo -e "${CYAN}└─────────────────────────────────────────────────────────────────────────────────┘${NC}"
    echo ""
  fi

  # ═══════════════════════════════════════════════════════════════════════════
  # FOOTER
  # ═══════════════════════════════════════════════════════════════════════════
  echo -e "  ${BLUE}🌐 Portfólio:${NC} https://lucassr.dev          ${GREEN}📦 GitHub:${NC} https://github.com/lucassr-dev/.config"
  echo ""
}
